...
export class CertificateVerifierContract implements CertificateVerifierCircuits {
  ...
  async verifyDegreePrivacy(witness: CertWitness, minGpa: number): Promise<Uint8Array> {
    ...
    const isSecretEmpty = witness.studentSecret.every((b: number) => b === 0);
    const isIdEmpty = witness.studentId.every((b: number) => b === 0);
    if (isSecretEmpty) {
    ...
    // Hash studentSecret and studentId to produce a 32-byte nullifier
